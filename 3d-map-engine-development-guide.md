...
### 後續迭代(按序):A3 → B9 → C10 → B5 → A2 → A4 → B8 → B7 → B6 → B10 → B11 → B12 → B13 →(數據來源調研)→ A5 → B14 → B15 → C1 → C2 → C3 → C4 → C5 → C6 → C7 → C8 → C9
...
| C8 | 遊戲層 v3 — 據點爭奪 + 陣營資源 + AI 對手 | `game/src/simulation.ts`(sites/factions/`trainUnit`)、`game/src/ai.ts`(新檔,`createAiController`)、`three/src/gameView.ts`(`siteObject?`/`neutralSiteColor?`)、`demo/src/gameSetup.ts`(**新檔**,從 `App.tsx` 抽出並升級)、`demo/src/App.tsx`、`e2e/game.spec.ts` + 各自測試 | 分四個工作項目交付,前三項各自過獨立 QA 後即凍結不再改動(本條目只做第四項):①`simulation.ts` 加**可選**建構期集合——`sites?: SiteDefinition[]`(`id`/`position`/`ownerFactionId?`/`captureRadius`預設12/`captureTime`預設5/`income`預設1)與`factions?: FactionDefinition[]`(`id`/`resources`預設0/`income`預設0),兩者皆是 additive(省略 = 舊行為零影響,連事件流都不變)。`tick(dt)` 由三個 phase 擴成**五個固定順序 phase**:①鎖定 ②移動 ③傷害(不變)→ **④據點**(在 phase③ 之後的存活單位、post-movement 位置上判定:每個站在 `captureRadius` 內的非中立單位陣營算「在場」,在場陣營扣掉據點現任 owner 剩下的是「攻方」——**恰好一個攻方且 owner 不在場** = 該攻方 `captureProgress` 累加 `dt`(換攻方或首次攻佔會重置進度並發 `site:capture-started`),累滿 `captureTime` 翻轉 `ownerFactionId` 並發 `site:captured`;**owner 在場或兩個以上攻方** = 進度與攻方**凍結**、不發任何事件;**無攻方** = 進度按 `dt` 衰退,歸零則清除攻方)→ **⑤收入**(用 phase④ 之後的歸屬,每個已註冊陣營拿 `(baseIncome + 名下據點 income 總和)·dt`——**故本 tick 才被攻下的據點,同一 tick 就已經在發薪**)。一個 tick 的事件發射順序因此變成:移動事件 → `unit:combat` → 逐陣亡單位的 `unit:defeated`+`unit:removed` → 逐據點(宣告序)的 `site:capture-started`(若有)接 `site:captured`(若有)——**同一據點同一 tick 可以兩個事件都發**(攻方在 `captureTime≤dt` 的極端情況一次跨越)。`trainUnit(factionId, {siteId, cost, id?, unit?})`:扣款 + 在該據點位置生一個屬於該陣營的單位(`unit:spawned` 照常同步發);**陣營未註冊、據點不存在、據點非該陣營所有、`cost` 非法(非有限或負)、餘額不足**——五種情況一律回傳 `null` 且**不改動任何狀態、不發任何事件**;重複 `id` 才會拋錯——**不論顯式傳入還是省略、改由 `spawnUnit` 自動產生的候選 id**,兩者都先驗證過才扣款,故 id 衝突永遠**拋錯發生在扣款之前**。②新檔 `ai.ts` 匯出 `createAiController(sim, {factionId, decisionInterval?預設1, maxUnits?預設8, train?})`,回傳 `{factionId, update(dt)}`。**純、確定性**——決策只讀 `GameSimulation` 的公開狀態 + 自己的 sim-秒累加器,不碰 `Math.random()`/`Date.now()`,連 core 的 seeded RNG 都刻意不用。`update(dt)` 每幀呼叫一次,**必須早於同一幀的 `sim.tick(dt)`**,且傳同一個 `dt`;累加達 `decisionInterval` 才跑一輪決策(一輪最多一次,卡幀不會補跑),一輪固定三步:(1) 威脅掃描(用跑之前的單位快照,判斷己方哪些據點附近有敵方陣營單位在場)(2) 訓練最多一單位,選字典序最小的己方據點,`ownUnitCount < maxUnits` 且己方至少擁有一個據點才train(3) 命令每個 idle/arrived 的己方單位——受威脅的己方據點存在就去救,否則去最近一個非己方據點(XZ 距離,同距 id 較小者勝),**但已經站在該目標據點 `captureRadius` 內的單位不下令**,留在原地(避免對已就位的守軍每輪重發一次無意義的移動指令);(2)之後才重取一次單位快照給(3),讓本輪剛練出的單位當輪就會被指派。③`gameView.ts` 加 `GameViewOptions.siteObject?`/`neutralSiteColor?`:每個 site 一個場景物件 `game:site:{siteId}`,預設是半透明地面圓盤(半徑=`captureRadius`,色隨 owner)+ 子物件進度環 `game:site:progress:{siteId}`(只在 `captureProgress>0` 時可見,朝 `captureTime` 縮放增長,色隨 `capturingFactionId`);傳自訂 `siteObject` 時預設的每幀顏色/進度同步就不再套用,呼叫方要自己在自己的 render hook 裡讀 `site`(活引用)。④(本條目)demo 端把原本內嵌在 `App.tsx` 的 `?game=1` 區塊抽成獨立模組 `demo/src/gameSetup.ts`,凍結契約 `setupGameDemo(renderer, world, params): GameDemoHandles | null`(`{sim, view, ais, dispose()}`),**BFS 尋起點/6 個生成點/紅藍分派/凍結不變量(前兩個生成單位同陣營)全部逐字保留**,新增:據點依世界種類推導(demo 自己判斷,`@map-engine/game` 完全不知道歷史/程序生成世界的存在)——**歷史世界**(存在 id 以 `'city:'` 開頭的建築)每座城一個據點(`captureRadius:6, captureTime:3, income:5`——該世界 1 unit=1km,是特地為此尺度取的一組值),owner 由 `districtId` 去掉開頭 `'d:'` 而來,`'d:hist'` 視為中立;**否則(程序生成/OSM)**固定三個據點(`captureRadius:10, captureTime:3, income:5`):`site:red`(BFS 起點,屬紅)、`site:blue`(生成點裡離起點最遠的一個,屬藍)、`site:neutral`(離起點最近、但離每個生成點都超過 `captureRadius` 的可達節點,逼玩家真的派兵過去;找不到就退而求其次選一個離起點最近、只是不是生成點本身的節點)——兩個陣營 `{red: resources 0/income 1, blue: resources 60/income 1}` 一律註冊。AI **加掛在 `&ai=1`**(需同時有 `game=1`):只在此時建一個 `createAiController(sim, {factionId:'blue', train:{cost:50}})` 塞進 `handles.ais`;純 `?game=1` **一個 AI 都不建**,這保住了 C1~C7 四個既有 e2e 原封不動。AI 的 frame hook **必須在 `createGameView()` 之前註冊**——`renderer.onFrame` 內部用 `Set` 存回呼且依註冊序遍歷(已核實),故 AI 的 `update(dt)` 先跑、`createGameView` 自己掛的 `onFrame`(裡面呼 `sim.tick(dt)`)後跑,AI 這一幀下的指令才來得及在這一次 `tick` 生效;HUD(`data-testid="game-hud"`)靠動態插入的 `<style>` 元素定義 `.game-hud-panel` class(非 inline style):桌面版卡在下方提示列與小地圖之間的縫隙(右下角);<900px 時提示列與小地圖都隱藏,改落腳左下角,`z-index:28` 蓋過手機版側欄的 27,確保手動重開側欄時 HUD 仍可讀;`textContent` 刷新**共用同一個 frame hook**(所以讀到的是上一幀的資源/據點數,慢半幀,肉眼不可見),`dispose()` 一併移除該 stylesheet 與 HUD 元素。`App.tsx` 除舊有 `__mapEngine.game`/`gameView` 外新增 `__mapEngine.gameAis`。e2e 新增 2 個測試(`game.spec.ts` 4→6):場景每個 `sim.listSites()` 都有對應 `game:site:{id}` 物件 + HUD 可見 + 派紅方單位走到 `site:neutral` 直到 `getSite().ownerFactionId==='red'`;`?ai=1` 下輪詢直到藍方單位數超過起始值**且**同一快照裡有藍方單位 `state` 為 `'moving'`/`'fighting'`,證明 AI 真的訓練並下達了指令。單元測試(全部落在已凍結的前三項,本條目未新增):`simulation.test.ts` 25→41、新檔 `ai.test.ts` 11,game 包由 41→68,單元測試基線 191/17 檔 → **218/18 檔**;e2e 總數 141 → **147**(每 project 47→49),spec 檔案數不變(仍 18 個)。 |
| C9 | 三國戰役 MVP — 荊州爭奪戰(219) | `demo/src/gameSetup.ts`(**重寫**,`GameController`)、`demo/src/game/scenario.ts`(新檔,`JINGZHOU_219`)、`demo/src/game/GameUI.tsx`+`gameUi.css`(新檔)、`three/src/gameView.ts`(`pickSite`/`healthBars`)、`demo/src/App.tsx`、`e2e/game.spec.ts`(**重寫**)、`demo/src/game/scenario.test.ts`(新檔) | **把 C1~C8 逐步搭出來的通用遊戲層,收斂成一個真正能玩完一局的三國戰役**——舊的程序生成沙盒(BFS 生成點、`site:red`/`site:blue`/`site:neutral`、`&ai=1`)**整個刪除**,`?game=1` 現在只在 `?map=three-kingdoms` 生效(`isGameModeUrl(params) = params.get('game')==='1' && params.get('map')==='three-kingdoms'`),非歷史地圖上 `setupGameDemo` 直接回傳 `null`——連 lobby 都不建,完全惰性。①新檔 `game/scenario.ts` 定義 `Scenario` 型別 + 常量 `JINGZHOU_219`:三勢力——曹操(`wan`/`xiangyang`,2 城)、劉備(`jiangling`/`yiling`/`shangyong`,3 城)、孫權(`changsha`/`chibi`/`wuchang`,3 城),`unitsPerCity:1`、`resources:60`/`income:1`、`siteDefaults:{captureRadius:6, captureTime:5, income:2}`、`trainCost:50`、`unitStats:{speed:30, hp:100, attackDamage:10, attackRange:8}`,連中文開場白與勝敗文案都在資料裡。②`gameSetup.ts` 整份重寫成 `GameController`:狀態機 `phase: 'lobby' \| 'playing' \| 'won' \| 'lost'`,`getState()`/`subscribe()` 給一個 UI 可觀察的快照(`factions[]`/`selection`/`canTrain`/`lastCommand`,靠 fingerprint 字串比對避免每幀都通知訂閱者)、`getCityLabels()` 給城池名牌的螢幕投影座標(每幀由 UI 自己的 rAF 呼叫,不進 React state)、`start(playerFactionId)` 才真正建 `sim`+`view`+AI+生單位+`focusPoint` 對準戰場(lobby 階段完全惰性,不建任何東西)、`train()`(在選中的己方城池練兵)、`clearSelection()`、`dispose()`。勝負判定:玩家佔滿全部 8 城 = `won`;玩家 0 城 0 兵 = `lost`;兩者皆終局態,一進去就再也不變(連 AI 的 frame hook 也一併凍結,但模擬本身照跑,遮罩背後的世界不會靜止)。③新檔 `GameUI.tsx`(+`gameUi.css`)取代 `AtlasUI`——**遊戲模式下兩者互斥掛載**,徹底不碰 `@map-engine/ui`;凍結的 `data-testid`:`game-lobby`、`game-faction-card-{id}`、`game-topbar`、`game-selected-panel`、`game-train-btn`、`game-end-overlay`、`game-restart-btn`、`game-city-label-{cityId}`。④`gameView.ts` 加 `pickSite(pointer): string \| null`(`pickUnit` 的鏡像——對 site marker 的獨立反查表 raycast,兩個反查表刻意互斥,單位站在據點圓盤上時兩個 pick 各自命中各自的目標而不互相遮擋)與 `GameViewOptions.healthBars?`(每單位一條浮動雙 sprite 血條,命名 `game:hp:{unitId}`,只在 `hp<maxHp` 時顯示,`raycast=()=>{}` 排除在 pick 之外,不拖慢 `pickUnit`/`pickSite`)。⑤`App.tsx`:`__mapEngine.gameController` 在遊戲模式下**一律**掛上(哪怕還在 lobby),`__mapEngine.game`/`gameView`/`gameAis` 只在 `start()` 之後才存在——這條契約被 e2e 逐字驗證。⑥`e2e/game.spec.ts` 整份重寫,見下方 e2e 段落。**兩個中途踩坑(值得記一筆)**:一是 `spawnUnit`/`moveUnitTo` 的 `position`/`target` 是 `Vec2`,`.y` 對應世界 z 而非世界高度——照抄 `Site.position`(`Vec3`)的 `.y` 會把單位傳送到錯誤高度,e2e 開發過程中曾為此排查一小時;二是 lobby 畫面一度直接顯示 `FactionStatus.unitCount`(`start()` 前恆為 0,`gameSetup.ts` 自己的文件化契約),於是三張勢力卡在開戰前全部顯示「兵力 0」——數值上沒錯但視覺上像壞掉,修法是 `GameUI.tsx` 改用 `scenario.factions` 自己算 `cities.length * unitsPerCity` 當 lobby 預覽值,`start()` 之後才換回即時 `unitCount`。單元測試新增 `demo/src/game/scenario.test.ts`(6):基線 218/18 檔 → **224/19 檔**;e2e `game.spec.ts` 6→4(見下)。 |
...
- [ ] `pnpm install && pnpm typecheck && pnpm lint && pnpm test` 全綠(基線 **224 unit / 19 檔**;C1 增 game 包的 pathfinding + simulation 共 23,C3 增 `three/terrainMesh.test.ts`(6)並把 historical 測試從 6 擴到 29,C4 再把 historical 擴到 38,C5 新增 `three/buildingsMesh.test.ts`(14),C6 把 buildingsMesh 測試擴到 21、historical 擴到 40,C7 把 game 包從 23 擴到 41(`pathfinding.test.ts` 10→16、`simulation.test.ts` 13→25),C8 把 game 包從 41 擴到 68(`simulation.test.ts` 25→41、新檔 `ai.test.ts` 11),單元測試檔案數 17→18,C9 新增 `demo/src/game/scenario.test.ts`(6)驗證 `JINGZHOU_219` 資料完整性,218/18 檔 → 224/19 檔)
- [ ] `npx playwright install chromium`(首次)後 `npx playwright test --workers=2` 全綠(**18 個 spec 檔、141 個測試**= 每個 viewport 47 個 × desktop/large/mobile 三個 project;C8 把 `game.spec.ts` 4→6,141→147;C9 把舊 6 個程序生成測試整份刪除、重寫成 4 個劇本測試,147→**141**(每 project 49→47),spec 檔案數不變(仍 18 個);3 viewport;本機別開太多 workers,見 §5.5)
