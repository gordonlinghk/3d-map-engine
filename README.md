...
The [`@map-engine/game`](packages/game) package turns a `MapWorld` into a playable substrate — the foundation for building an actual game on the engine. It is pure, deterministic, zero-dependency TypeScript (no DOM/Three.js): **A\* pathfinding** over the road graph (optimal, deterministic routes; an optional `edgeCost` hook re-weights or closes off edges — clamped to keep the Euclidean heuristic admissible, so A\* stays optimal), **units** that move along those routes hugging the terrain and can belong to a **faction** and fight (deterministic three-phase combat tick — engage, move-or-hold, damage — emitting `unit:combat`/`unit:defeated`), **capturable sites** that flip ownership after a faction holds the ground alone for a configurable time (contested presence freezes progress, an empty site decays it) and pay their owner income every tick, **factions** that hold resources and can `trainUnit()` a new unit at an owned site for a resource cost, and an **event-driven simulation** (`unit:spawned` / `waypoint` / `arrived` / `combat` / `defeated` / `removed` / `site:capture-started` / `site:captured`). A deterministic **AI controller** (`createAiController`) can play a faction end-to-end — training at its cheapest owned site and sending idle units to defend threatened sites or expand to unclaimed ones — driven by nothing but the simulation's own public state (no RNG, no wall clock). The Three.js binding `createGameView` (in `@map-engine/three`) renders a marker per unit (optionally colored per faction) plus a progress-ringed marker per site, floating health bars over damaged units, offers **camera-follow**, and supports **click-to-select** (`pickUnit`/`pickSite`, a ring highlights the selected unit).

Try it in the demo with **`?map=three-kingdoms&era=y219&game=1`** — **荊州爭奪戰(建安二十四年)**, a full scenario battle built on the Three Kingdoms historical map: pick 曹操, 劉備 or 孫權 in the lobby (each starts holding 2–3 cities and a matching number of soldiers) and try to capture all 8 of Jingzhou's cities — 宛/襄陽 (曹操), 江陵/夷陵/上庸 (劉備), 長沙/赤壁/鄂 (孫權). Click a unit to select it, click the ground to send it marching (auto-fighting on contact with an enemy); click your own city to open its panel and train a new soldier there for resources. The other two factions are played by the deterministic AI opponent. Capture every city to win, lose all your cities and units to lose. `?game=1` does nothing at all on procedural or OSM maps — it's gated to the historical Three Kingdoms map only, so the default demo is untouched.
...
| `@map-engine/game` | Game logic: A\* pathfinding over the road graph (optional weighted/impassable edges), deterministic unit movement, faction combat, capturable sites + faction resources/`trainUnit`, a deterministic AI controller, event-driven simulation | core (no DOM/Three.js) |
